...
def build_potongan_gaji(batch_root_id: str) -> None:
    ...
    LOGGER.info("Starting phase4: build potongan gaji for batch ID {}".format(batch_root_id))
    ...
    _generate_excel(batch_root_id, organisasi_df, daftar_potongan_gaji_df)

    end_time = datetime.now()
    LOGGER.info(f"build potongan gaji finished in {end_time - start_time}")


def _generate_excel(batch_root_id: str, organisasi_df: pd.DataFrame,
